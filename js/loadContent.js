...
export const LoadContent = {
  ...
  async loadBestRating(page) {
    const res = await fetch(`${Config.API_HOST}/discover/tv?language=pt-BR&sort_by=vote_average.desc&vote_count.gte=200&page=${page}&include_adult=${Config.INCLUDE_ADULT}`, {
      headers: { Authorization: `Bearer ${Config.TOKEN}` }
    }).then(res => res.json());
  ...
  async loadMoviesByCategory(categoryName, page) {
    ...
    const byCategory = res.results.map(m => ({ ...m, media_type: 'movie' }));
